...
class NuscVideoSampler(DefaultSampler):
    def __init__(self, dataset, batch_size=None, cam_num=6, frame_num=32, hz_factor=1, video_split_rate=2,
                 mv_video=False, view='cam_front', shuffle=False, infinite=True, seed=6666, logger=None,
                 resample_num_workers=8, resample_batch_size=64):
        super(NuscVideoSampler, self).__init__(dataset, batch_size=batch_size, shuffle=shuffle, infinite=infinite,
                                               seed=seed)
        self.view = view
        ...
        logger.info(
            'Sampling video data from image dataset (depends on num_frames, hz_factor, video_split_rate), this may take minutes...')
        logger.info('For faster debugging, please use mini-version of nuscene.')
        dataloader = DataLoader(dataset, batch_size=resample_batch_size, num_workers=resample_num_workers,
        ...
        for batch in tqdm(dataloader):
            ...
            flags = (frame_idxs % (video_frame_len // video_split_rate) == 0) & (
                        frame_idxs + video_frame_len <= video_lengths)
        ...
        if self.mv_video:
            self.total_size = int(math.ceil(
                len(self.index) / self.cam_num / video_batch_size)) * video_batch_size * self.data_num_per_batch
        else:
    ...
    def __iter__(self):
        ...
        while True:
            ...
            ...
            """
            new_indices = np.stack([np.array(indices) + i * self.hz_factor for i in range(self.frame_num)]).T.reshape(
                -1)
